...
| Mode          | Steps                                                                |
| ------------- | -------------------------------------------------------------------- |
| **Full**      | Parse → Setup → Plan → Generate → Heal → Snapshot & Report → Summary |
| **Heal-Only** | Parse → Setup → Plan → Heal → Snapshot & Report → Summary            |
...
→ Step 5.5 (when `phase="healing-complete"`)

---

## Step 5.5: Snapshot & Report Generation (All Modes)

### Preconditions

- `session.json` exists with `phase="healing-complete"`
- Test files exist in `playwright-tests/ai-generated/`
- All tests have been executed (passed or failed)

### Execute

#### Part A: Run Tests with Snapshots

Run Playwright tests with screenshot capture enabled to generate per-test-case snapshots:
...
```typescript
use: {
  screenshot: 'only-on-failure',  // or 'on' for all screenshots
  trace: 'retain-on-failure',
  video: 'retain-on-failure',
}
```

**Snapshot Output Directory:** `playwright-output/screenshots/`

Each test case should include a `page.screenshot()` call:
...
#### Part B: Generate Playwright HTML Report

After test execution completes, generate the HTML report:
...
#### Part C: Verify Snapshot & Report Artifacts

Verify that all expected artifacts were generated:

- [ ] `playwright-report/index.html` exists (HTML report)
- [ ] `playwright-output/screenshots/` contains screenshots for each test case
- [ ] `playwright-output/test-results/` contains trace/video files (if configured)
- [ ] HTML report is accessible and displays test results correctly

**If artifacts are missing:**

1. Re-run tests with explicit screenshot paths
2. Verify `playwright-output/screenshots/` directory exists and is writable
3. Check that `page.screenshot()` calls are present in each test case
...
```json
{
  ...
  "artifacts": {
    ...
    "screenshots": "playwright-output/screenshots/",
    "jsonResults": "playwright-output/test-results/",
...
→ Step 6 (when `phase="report-complete"`)
