...
### 5.1.1: Generate Snapshots & HTML Report (After Each Test Run)

After running tests, generate snapshots and the Playwright HTML report:

**Step 1: Run tests with HTML + JSON reporters and screenshot output:**
...
**Step 2: Verify snapshot artifacts exist:**

Check that `playwright-output/screenshots/` contains a `.png` file for each test case ID (TC-XXXX.png).

If any snapshots are missing, verify that the test code includes `page.screenshot()` calls:
...
**Step 3: Generate and serve HTML report:**
...
**Step 4: Update session.json with report artifacts:**
